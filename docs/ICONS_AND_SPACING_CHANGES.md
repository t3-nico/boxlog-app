...
#### Createボタンの内部構造
```tsx
<Button className="w-full h-[56px] py-4 px-4 flex items-center justify-between">
  <div className="flex items-center gap-2">        {/* アイコン-テキスト間隔 */}
...
#### ナビゲーション矢印のパディング
```tsx
<button className="p-1.5 rounded-full">           {/* コンパクトなパディング */}
  <ChevronLeft className="h-5 w-5" />
</button>
```
...
#### ナビゲーション矢印
```tsx
<button className="p-1.5 rounded-full">  // 円形維持（クリック領域明確化）
  <ChevronLeft />
</button>
```
