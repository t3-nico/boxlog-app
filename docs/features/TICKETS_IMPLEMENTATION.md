...
- `plans` テーブル：チケット情報（タイトル、説明、ステータス、優先度等）
...
```typescript
// plans
api.plans.list // チケット一覧取得（フィルタ対応）
api.plans.getById // チケット詳細取得
api.plans.create // チケット作成
api.plans.update // チケット更新
api.plans.delete // チケット削除
...
- `src/features/plans/stores/useplanStore.ts` - チケット状態管理
- `src/features/plans/stores/useSessionStore.ts` - セッション状態管理

**カスタムフック:**

- `src/features/plans/hooks/useplans.ts` - チケットCRUD操作
