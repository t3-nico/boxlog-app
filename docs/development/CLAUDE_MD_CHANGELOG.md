...
- ✅ 「セキュリティ」セクションをSkill参照型に変更
  - クイックチェック（4項目）+ `.claude/skills/security/SKILL.md`への参照
- ✅ 「アクセシビリティ」セクションをSkill参照型に変更
  - クイックチェック（4項目）+ `.claude/skills/a11y/SKILL.md`への参照
- ✅ 「React最適化パターン」にVercelベストプラクティス参照追加
  - `.claude/skills/react-best-practices/SKILL.md`への参照（45ルール）
- ✅ 「テスト実装」セクションを簡素化
  - `.claude/skills/test/SKILL.md`への参照追加
  - 重複するテストコマンドセクションを削除

**理由**:

- CLAUDE.mdの肥大化を防ぎ、コンテキスト効率を維持
- 詳細な実装ガイドはSkillに集約し、CLAUDE.mdは「何を」「なぜ」に集中
- Skillの自動発動機能を活用し、必要時に詳細ガイドを参照

**効果**: CLAUDE.md 837行 → 778行（約7%削減）、Skillとの役割分担明確化
