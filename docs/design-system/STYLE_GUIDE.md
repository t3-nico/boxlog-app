...
| ------------------- | -------------------------------- | ------------------------------ | ------ |
| 塗りボタンホバー    | `--color-primary-hover`          | `hover:bg-primary-hover`       | 90%    |
| コンテナ/バッジ背景 | `--color-primary-container`      | `bg-primary-container`         | 12%    |
| 選択状態            | `--color-primary-state-selected` | `bg-primary-state-selected`    | 12%    |
| ホバー状態          | `--color-primary-state-hover`    | `hover:bg-primary-state-hover` | 10%    |
...
// ❌ 禁止 → ✅ 代替
hover:bg-primary/90  → hover:bg-primary-hover
hover:bg-primary/10  → hover:bg-primary-state-hover
hover:bg-primary/8   → hover:bg-primary-state-hover
hover:bg-primary/5   → hover:bg-primary-state-hover
bg-primary/10（装飾）→ bg-primary-container
bg-primary/12（選択）→ bg-primary-state-selected
text-primary         → text-on-primary-container（container上の場合）
```

### 例外ケース（許容）

以下のケースはセマンティックトークンが存在しないため、直接透明度指定を許容します：

| パターン                             | 用途                   | 理由                   |
| ------------------------------------ | ---------------------- | ---------------------- |
| `bg-primary/50`                      | 現在時刻線の非今日表示 | 50%の特別な用途        |
| `bg-primary/20`, `/40`, `/60`, `/80` | ヒートマップの段階色   | 複数段階の視覚化が必要 |
| OKLCHインライン（動的計算）          | 時間選択のドラッグ範囲 | CSSで動的計算が必要    |
...
| 用途             | トークン                          | Tailwindクラス                     | 透明度 |
| ---------------- | --------------------------------- | ---------------------------------- | ------ |
| コンテナ背景     | `--color-destructive-container`   | `bg-destructive-container`         | 12%    |
| ホバー状態       | `--color-destructive-state-hover` | `hover:bg-destructive-state-hover` | 10%    |
| 塗りボタンホバー | `--color-destructive-hover`       | `hover:bg-destructive-hover`       | 90%    |

```tsx
// ✅ エラーメッセージのコンテナ
<div className="bg-destructive-container text-destructive rounded-lg p-3">
  エラーが発生しました
</div>

// ✅ 削除ボタン（ゴーストバリアント）
<Button className="text-destructive hover:bg-destructive-state-hover">
  削除
</Button>
...
| 用途          | トークン                        | Tailwindクラス             | 透明度 |
| ------------- | ------------------------------- | -------------------------- | ------ |
| コンテナ背景  | `--color-accent-container`      | `bg-accent-container`      | 10%    |
| 選択/強調状態 | `--color-accent-state-selected` | `bg-accent-state-selected` | 20%    |

```tsx
// ✅ 睡眠セクションの背景
<div className="bg-accent-container">睡眠時間帯</div>

// ✅ 睡眠時間のハイライト
<div className="bg-accent-state-selected">22:00 - 6:00</div>
```

#### Secondary（Switchコンポーネント等）

| 用途       | トークン                  | Tailwindクラス             | 透明度 |
| ---------- | ------------------------- | -------------------------- | ------ |
| ホバー状態 | `--color-secondary-hover` | `hover:bg-secondary-hover` | 80%    |
...
// Destructive
bg-destructive/10, /12     → bg-destructive-container
hover:bg-destructive/10    → hover:bg-destructive-state-hover
hover:bg-destructive/90    → hover:bg-destructive-hover

// Accent
bg-accent/10               → bg-accent-container
bg-accent/20               → bg-accent-state-selected

// Secondary
hover:bg-secondary/80      → hover:bg-secondary-hover
```
...
| パターン                 | 用途               | 理由                     |
| ------------------------ | ------------------ | ------------------------ |
| `bg-destructive/60`      | オーバーラップ警告 | 特殊な視覚フィードバック |
| `bg-destructive/80`      | 通知バッジ         | 特殊な強調色             |
| `dark:bg-destructive/60` | ダークモード調整   | モード別の微調整         |
| `bg-muted/50`            | 非睡眠セグメント   | 薄い背景が必要           |
