      ...
      endpoints.forEach((endpoint) => {
        if (endpoint && typeof endpoint === 'object') {
          ...
          const avgResponseTime =
            'averageResponseTime' in endpoint ? endpoint.averageResponseTime : 0;
...
async function testCors(baseUrl = API_CONFIG.baseUrl) {
  ...
  try {
    ...
    const allowOrigin =
      'access-control-allow-origin' in headers ? headers['access-control-allow-origin'] : null;
    const allowMethods =
      'access-control-allow-methods' in headers ? headers['access-control-allow-methods'] : null;
    const allowHeaders =
      'access-control-allow-headers' in headers ? headers['access-control-allow-headers'] : null;
...
function makeRequest(method, url, headers = {}) {
  return new Promise((resolve, reject) => {
    ...
    const options = {
      ...
      method: method,
      headers: {
        'User-Agent': 'Dayopt-API-Version-Manager/1.0',
        ...headers,
      },
...
async function main() {
  ...
  logger.header(`Dayopt API Version Manager`);
