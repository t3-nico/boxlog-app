...
const CONFIG = {
  ...
  errorCategories: {
    ...
    low: {
      patterns: ['Expected .* arguments, but got', 'This condition will always return', 'Unreachable code detected'],
      priority: 4,
      description: 'レビュー・検討',
      icon: '🟢',
    },
  ...
  // 除外パターン
  excludePatterns: ['src/test/**', '**/*.test.ts', '**/*.test.tsx', '**/*.d.ts', 'node_modules/**'],
...
function detectRegression(currentStats, baseline) {
  ...
  return {
    hasRegression: false,
    message: increase < 0 ? `改善: ${Math.abs(increase)}件のエラー削減` : '変化なし',
    details: {
      previous: previousTotal,
      current: currentTotal,
...
async function runTypeScriptStrictCheck() {
  ...
  if (regression.hasRegression) {
    ...
    console.log(
      `   📊 ${regression.details.previous} → ${regression.details.current} (+${regression.details.increase})`
    )
  }
...
function evaluateQualityGate(stats) {
  ...
  // ファイル別エラー数チェック
  const fileViolations = Object.entries(stats.byFile).filter(
    ([, data]) => data.count > CONFIG.thresholds.maxErrorsPerFile
  ).length
  ...
  return {
    ...
    message: violations.length === 0 ? 'すべての品質基準をクリア' : `${violations.length}件の品質違反`,
    violations,
  }
}
...
if (require.main === module) {
  ...
  const _updateBaseline = process.argv.includes('--update-baseline')
