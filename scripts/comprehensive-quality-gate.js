...
} catch (e) { console.log('⚠️  License checker not available'); }

try {
  apiChangeDetector = require('./api-change-detector.js');
} catch (e) { console.log('⚠️  API change detector not available'); }

try {
  performanceRegression = require('./performance-regression-test.js');
} catch (e) { console.log('⚠️  Performance regression tester not available'); }

try {
  secretDetector = require('./gitleaks-secret-detector.js');
} catch (e) { console.log('⚠️  Secret detector not available'); }

try {
  typescriptStrictChecker = require('./typescript-strict-mode-checker.js');
} catch (e) { console.log('⚠️  TypeScript strict checker not available'); }
  ...
  results.forEach(result => {
    ...
    systemEval.violations.forEach(violation => {
      if (violation.severity === 'critical' ||
          (violation.severity === 'high' && gateConfig.blockingLevel !== 'none')) {
...
function evaluateSystemResult(result, requirements) {
  ...
  try {
    switch (result.system) {
      ...
      case 'Secret Detection':
        const secretsFound = result.results.stats?.secretsFound || 0;
    ...
    console.log(`⚠️  Error evaluating ${result.system}:`, error.message);
  }
...
function generateRecommendations(evaluation) {
  ...
  // システム別推奨事項
  const failedSystems = evaluation.violations
    .filter(v => v.type === 'system_failure')
    .map(v => v.system);
...
async function runComprehensiveQualityGate() {
  ...
  try {
    // 品質ゲート設定決定
    const gateLevel = process.argv.includes('--enterprise') ? 'enterprise' :
