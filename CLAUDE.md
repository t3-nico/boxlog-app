...
PORT=4000 npm run dev  # ← 開発サーバーを1つだけ起動（絶対に複数起動しない）

# 2. コード記述中（常時）
# → test:watchがバックグラウンドで監視中
# → ファイル保存時に自動テスト実行
# → 開発サーバーは1つだけ動作中（PORT=4000）

# 3. エラー発生時（手動対処）
killall -9 node     # 全プロセス停止
rm -rf .next        # キャッシュクリア
PORT=4000 npm run dev  # クリーンに1つだけ再起動
...
1. **セッション開始時（AI自動実行）**: `npm run test:watch` をバックグラウンド起動
   ...
   - **重要**: test:watch は**セッション中1回だけ**起動（複数起動厳禁）
     1-2. **開発サーバー起動（AI自動実行）**: `PORT=4000 npm run dev` を1つだけ起動
   - **対象**: Claude Code（AI）がセッション開始時に自動実行
   - **方法**: `Bash`ツールで `run_in_background: true` オプション使用（**フォアグラウンド推奨**）
   - **ポート**: 必ず `PORT=4000` を使用（AIは4000番台、開発者は3000番台）
   - **厳守事項**:
     - ✅ **セッション中1つだけ**起動（複数起動は絶対禁止）
     - ❌ エラー発生時に新規プロセスを起動しない
     - ❌ 複数ポート（4000, 4001, 4002等）の同時使用禁止
     - ✅ エラー時は既存プロセス停止後、クリーンに再起動
   - **エラー対処**:
     1. 既存プロセス全停止: `killall -9 node` または `KillShell`
     2. キャッシュクリア: `rm -rf .next`
     3. 1つだけ再起動: `PORT=4000 npm run dev`
   - **ユーザーへの依頼**: 制御不能になった場合は、ターミナルで `killall -9 node` 実行を依頼
