...
| 場所               | importできるもの                                                                                            | 禁止                                         |
| ------------------ | ----------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| `src/features/**`  | 同一feature（相対パス）, `@/types/*`, `@/schemas/*`, `@/lib/*`, `@/hooks/*`, `@/stores/*`, `@/components/*` | `@/features/*`（他featureへの依存ゼロ）      |
| `src/app/**`       | `@/features/*`（**barrelのみ**）, `@/types/*`, その他全て                                                   | `@/features/*/components/*` 等の deep import |
| 共有層（下記参照） | `@/types/*`, `@/schemas/*`, `@/lib/*`, `@/hooks/*`, `@/stores/*`, `@/components/*`                          | `@/features/*`（逆依存禁止）                 |
...
| 場所              | 用途                            | 例                                              |
| ----------------- | ------------------------------- | ----------------------------------------------- |
| `src/types/`      | 複数featureが使う共有型         | CalendarEvent, Tag, Entry, Settings             |
