...
| このagent（回帰スキャン）           | `/security` skill（実装ガイド）        |
| ----------------------------------- | -------------------------------------- |
| 既存コード全体をPASS/FAILでスキャン | 新規コード実装時のセキュリティパターン |
| 過去に修正した問題の再発検出        | OWASP Top 10ベースのチェックリスト     |
| Grepコマンドによる機械的検出        | コード例付きの正しい実装パターン       |

**実装パターンの詳細は `/security` を参照。このagentは「正しい書き方」の提示ではなく、「違反の検出」に集中する。**
