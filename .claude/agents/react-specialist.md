...
| このagent（批評・評価）             | `/react-best-practices` skill（実装ガイド） |
| ----------------------------------- | ------------------------------------------- |
| 既存コードの設計品質を批評          | 新規コード実装時のルール参照                |
| 「なぜこの設計が良い/悪いか」を議論 | 「こう書くべき」パターン提供                |
| Agent Teamsでの対話的レビュー       | メインClaude実装中に自動発動                |

**実装ルールの詳細は `/react-best-practices`（45ルール）を参照。このagentはルールの機械的適用ではなく、設計判断の妥当性評価に集中する。**
