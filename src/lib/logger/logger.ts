...
export class Logger {
  ...
  security(
    message: string,
    security: SecurityLogEntry['security'],
    meta: Record<string, unknown> = {},
  ): void {
    ...
    const level: LogLevel =
      security.threatLevel === 'critical' || security.threatLevel === 'high' ? 'error' : 'warn';
  ...
  business(
    message: string,
    business: BusinessLogEntry['business'],
    meta: Record<string, unknown> = {},
  ): void {
  ...
  private setupDefaultOutputs(): void {
    ...
    this.outputs = this.outputs.filter(
      (output) => !['console', 'file', 'rotating-file'].includes(output.name),
    );
    ...
    if (this.config.file.enabled) {
      ...
      this.outputs.push(
        new RotatingFileOutput(this.config.file.path, 'json', this.config.file.rotation),
      );
    }
  }
}
