...
```tsx
const meta = {
  title: 'Primitives/Button',   // Sidebar上のパス
...
| パーツ | 説明 |
|--------|------|
| **Sidebar** | 左側の Story ツリー。Meta の `title` 階層（例: `Primitives/Button`）で構成される。Dayopt では `storySort` で `Docs > Foundations > Primitives > Recipes > Features > Patterns` の順に並べている。 |
