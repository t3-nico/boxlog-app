...
| カテゴリ | 内容 | 例 |
|---------|------|-----|
| **Foundations** | デザイントークン・設計原則 | Colors, Typography, Spacing, Elevation |
| **Primitives** | 単体UIコンポーネント | Button, Badge, Input, Dialog |
| **Recipes** | 2つ以上のPrimitiveを組み合わせた複合パターン | ActionFooter, Field, ConfirmDialog, Inspector/* |
| **Features** | ドメインロジックを含むFeatureコンポーネント | Plans/*, Calendar/*, Tags/* |
| **Patterns** | 実装パターンのドキュメント | Forms, Feedback, Loading |
| **Docs** | ガイドライン・アーキテクチャ | StyleGuide, Accessibility |
...
| プレフィックス | 例 |
|---------------|-----|
| `bg-*` | `bg-background`, `bg-surface-inset`, `bg-container`, `bg-card` |
...
公式テンプレート: `Primitives/AlertDialog`
...
- `Primitives/Button` → 単体UIコンポーネント
- `Recipes/ActionFooter` → 複合パターン
- `Features/Plans/PlanInspector` → Featureコンポーネント
- `Docs/はじめに` → ドキュメント
