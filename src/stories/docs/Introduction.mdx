...
## ⚠️ 運用ルール：Storybookが正（Single Source of Truth）
...
| **新しいパターンが必要 = 先にStoryを追加** | 新しい使い方をする前に、まずStoryを追加してレビューする |

### なぜこのルールがあるのか

1. **一貫性**: 全員が同じパターンを使うことでUIの一貫性を保つ
2. **ドキュメント**: Storybookを見れば「何が使えるか」が分かる
3. **品質管理**: 使用パターンを限定することでテスト・レビューの負荷を下げる

### 実践例

```tsx
// ✅ OK: Storybookに記載されているパターン
<Button variant="ghost" size="icon">
  <Settings className="size-4" />
</Button>

// ❌ NG: Storybookに記載されていないパターン
<Button variant="link" size="xl">  // size="xl"はStoryにない
  リンク
</Button>
```
...
Tailwindクラス名で統一。コードにそのままコピペ可能。

| プレフィックス | 用途 | 例 |
|---------------|------|-----|
| `bg-*` | 背景色 | `bg-background`, `bg-card` |
| `text-*` | テキスト色 | `text-foreground`, `text-muted-foreground` |
| `border-*` | ボーダー色 | `border-border` |
| `ring-*` | フォーカスリング色 | `ring-ring` |
