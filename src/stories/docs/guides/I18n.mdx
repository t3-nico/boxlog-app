...
各JSONファイルは**ネームスペース名と一致するトップレベルキー**を持つ。`src/platform/i18n/request.ts` が全ファイルを `Object.assign` でフラットにマージするため、トップレベルキーがそのままグローバルなネームスペースになる。
...
3. `src/platform/i18n/request.ts` の `NAMESPACES` 配列に追加
...
- [ ] `src/platform/i18n/request.ts` の `NAMESPACES` に追加
