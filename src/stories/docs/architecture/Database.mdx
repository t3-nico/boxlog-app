...
| テーブル           | 役割                              | 主要カラム                                                                |
| ------------------ | --------------------------------- | ------------------------------------------------------------------------- |
| **plans**          | タスク/プラン（タイムボクシング） | title, status(open/closed), start_time, end_time, recurrence_*            |
...
| **plan_instances** | 繰り返しプランの個別インスタンス  | plan_id, instance_date, exception_type, title, description, instance_start, instance_end |
...
| `cleanup-plan-activities`   | 毎日 03:30 UTC | 365日            | plan_activities   |
| `cleanup-record-activities` | 毎日 03:40 UTC | 365日            | record_activities |
