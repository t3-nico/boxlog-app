...
import { normalizeDateTimeConsistency, removeUndefinedFields } from '@/server/api/routers/plans/utils'
...
export class PlanService {
  ...
  async list(options: ListPlansOptions): Promise<PlanWithTags[]> {
    const { userId, tagId, status, search, sortBy = 'created_at', sortOrder = 'desc', limit, offset } = options
  ...
  private async getPlanIdsByTagId(tagId: string): Promise<string[]> {
    const { data, error } = await this.supabase.from('plan_tags').select('plan_id').eq('tag_id', tagId)

    if (error) {
      throw new PlanServiceError('TAG_FILTER_FAILED', `Failed to apply tag filter: ${error.message}`)
    }
  ...
  private formatPlanWithTags(plan: PlanWithTags): PlanWithTags {
    const tags = plan.plan_tags?.map((pt) => pt.tags).filter((t): t is NonNullable<typeof t> => t !== null) ?? []
  ...
  private async getExistingPlan(planId: string, userId: string): Promise<PlanRow | null> {
    const { data } = await this.supabase.from('plans').select('*').eq('id', planId).eq('user_id', userId).single()
   ...
   * 変更を追跡
   */
  private async trackChanges(planId: string, userId: string, oldData: PlanRow, newData: PlanRow): Promise<void> {
