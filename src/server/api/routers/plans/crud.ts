...
export const plansCrudRouter = createTRPCRouter({
  list: protectedProcedure.input(planFilterSchema.optional()).query(async ({ ctx, input }) => {
    ...
    // Filter by tag ID (JOIN with plan_tags)
    if (input?.tagId) {
      ...
      if (planIdsError) {
        throw new TRPCError({
          ...
          message: `Failed to apply tag filter: ${planIdsError.message}`,
        })
      }
  ...
  create: protectedProcedure.input(createPlanSchema).mutation(async ({ ctx, input }) => {
    ...
    // Apply normalized values back to input
    ...
    const insertData = {
      user_id: userId,
      plan_number: '', // Auto-generated by trigger
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      console.debug('[plans.update] Update request:', {
        id: input.id,
        data: input.data,
        userId,
      })
      ...
      console.debug('[plans.update] Current data:', oldData)

      // Ensure date consistency (only if date/time fields are updated)
      ...
      console.debug('[plans.update] Updated data:', data)

      // Record activity: detect and track changes
      if (oldData) {
  ...
  delete: protectedProcedure.input(planIdSchema).mutation(async ({ ctx, input }) => {
    ...
    // Get plan info for activity recording
    ...
    // Record activity: plan deleted (before deletion)
    await supabase.from('plan_activities').insert({
      plan_id: input.id,
      user_id: userId,
      action_type: 'deleted',
