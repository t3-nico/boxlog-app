 ...
 * プランの重複計算
 */
function calculatePlanOverlaps(
  plans: CalendarPlan[],
  dateRange: { start: Date; end: Date },
): OverlapResult[] {
  ...
  const relevantPlans = plans.filter(
    (plan) =>
      plan.startDate && plan.startDate >= dateRange.start && plan.startDate <= dateRange.end,
  );
...
function groupPlansByTime(plans: CalendarPlan[]): CalendarPlan[][] {
  ...
  for (const plan of sortedPlans) {
    ...
    const overlappingGroup = groups.find((group) =>
      group.some((groupPlan) => plansOverlap(plan, groupPlan)),
    );
