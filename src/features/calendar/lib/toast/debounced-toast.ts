...
export const useDebouncedToast = () => {
  ...
  const groupedToasts = useRef<
    Map<string, { count: number; lastMessage: string; toastId?: string | undefined }>
  >(new Map());
  ...
  const createDebouncedFunction = useCallback(
    <Args extends unknown[]>(
      ...
      return (...args: Args): string | number | undefined => {
        ...
        // maxWait による強制実行判定
        const shouldInvokeByMaxWait =
          maxWait && state.lastInvokeTime && now - state.lastInvokeTime >= maxWait;
  ...
  const groupedToast = useCallback(
    (
      ...
      const group = groupedToasts.current.get(groupConfig.key) || {
        count: 0,
        lastMessage: '',
        toastId: undefined,
      };
