...
export const useDebouncedToast = () => {
  ...
  useEffect(() => {
    return () => {
      debounceStates.current.forEach((state) => {
        if (state.timeoutId) {
          clearTimeout(state.timeoutId)
        }
      })
      debounceStates.current.clear()
      groupedToasts.current.clear()
    }
  }, [])
  ...
  const createDebouncedFunction = useCallback(
    <Args extends any[]>(
      ...
      return (...args: Args): string | undefined => {
        ...
        // maxWait による強制実行判定
        const shouldInvokeByMaxWait = maxWait && 
          state.lastInvokeTime && 
          (now - state.lastInvokeTime) >= maxWait
  ...
  const debouncedSuccess = useCallback((
    ...
    config: DebounceConfig = { delay: 500 }
  ) => {
  ...
  const debouncedWarning = useCallback((
    message: string,
    options?: CalendarToastOptions,
    config: DebounceConfig = { delay: 400 }
  ) => {
...
export const TOAST_GROUPS = {
  BULK_OPERATIONS: createToastGroupConfig(
    'bulk-operations',
    3,
    (count) => `${count}件の操作を実行しました`
  ),
  
  SYNC_OPERATIONS: createToastGroupConfig(
    'sync-operations',
    2,
    (count) => `${count}件の同期操作を実行しました`
  ),
  
  VALIDATION_ERRORS: createToastGroupConfig(
    'validation-errors',
    2,
    (count) => `${count}件の入力エラーがあります`
  )
}