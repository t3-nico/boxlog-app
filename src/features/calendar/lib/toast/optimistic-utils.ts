...
// 結果をマージする関数
export const mergeOptimisticResults = <T>(results: OptimisticUpdateResult<T>[]): OptimisticUpdateResult<T[]> => {
  ...
  const executeQueued = useCallback(
    async <T>(
      ...
      for (const op of operations) {
        const result = await withOptimisticUpdate(op.optimisticUpdate, op.actualUpdate, op.rollback, op.options)
        results.push(result)
