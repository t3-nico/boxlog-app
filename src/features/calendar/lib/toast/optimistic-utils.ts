  ...
  const withOptimisticUpdate = useCallback(
    async <T>(
      ...
      const {
        ...
        enableRetry = true,
        operationDescription: _operationDescription = getTranslation(
          CALENDAR_TOAST_KEYS.TOAST_OPERATION,
        ),
      } = options;
      ...
      try {
        ...
        if (errorCount === 0 && successMessage) {
          ...
          if (errorMessage) {
            ...
            toast.error(
              `${errorCount}${getTranslation(CALENDAR_TOAST_KEYS.TOAST_OPERATION_FAILED)}`,
            );
          }
        }
