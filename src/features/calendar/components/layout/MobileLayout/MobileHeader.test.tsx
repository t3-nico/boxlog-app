...
describe('MobileHeader', () => {
  ...
  describe('戻るボタン', () => {
    ...
    it('showBackButton=falseでメニューボタンが表示される', () => {
      renderWithIntl(
        <MobileHeader {...defaultProps} showBackButton={false} onMenuToggle={vi.fn()} />,
      );
  ...
  describe('ビュー切り替えメニュー', () => {
    ...
    it('現在のビューがハイライトされる', () => {
      const mockViewChange = vi.fn();
      renderWithIntl(
        <MobileHeader {...defaultProps} viewType="week" onViewChange={mockViewChange} />,
      );
      ...
      const menuWeekButton = buttons.find(
        (b) => b.textContent === '週表示' && b.classList.contains('w-full'),
      );
