...
const AccessibilityHeader = ({
  ...
  >
    <div>
      <Dialog.Title
        className={cn('text-foreground text-2xl font-bold', getContrastClassName('text-foreground', 'contrast-text'))}
      >
        アクセシビリティ設定
      </Dialog.Title>
      <Dialog.Description
  ...
  <section>
    ...
    {isSystemHighContrast === true && (
      <div
        className={cn(
          ...
          getContrastClassName('border-primary/20 bg-primary/10', 'contrast-selected contrast-border')
        )}
      >
        <p className={cn('text-primary text-sm', getContrastClassName('text-primary', 'contrast-text'))}>
    ...
    <div className="mb-4 flex items-center justify-between">
      <label
        ...
        className={cn('text-foreground text-sm font-medium', getContrastClassName('text-foreground', 'contrast-text'))}
      >
        ハイコントラストモードを有効にする
      </label>
      ...
      <button
        ...
        className={cn(
          ...
          getContrastClassName(isHighContrastEnabled ? 'bg-primary' : 'bg-muted', 'contrast-selected')
        )}
      >
        <span
          className={cn(
                          ...
                          {theme.wcagAAA != null && (
                            <span
                              className={cn(
                                ...
                                getContrastClassName('bg-success/10 text-success', 'contrast-success')
                              )}
                            >
                              WCAG AAA
                            </span>
                  ...
                  className={cn(
                    ...
                    themeValidation.wcagAAA
                      ...
                      : themeValidation.wcagAA
                        ? getContrastClassName('border-warning/20 bg-warning/10', 'contrast-warning')
                        : getContrastClassName('border-destructive/20 bg-destructive/10', 'contrast-error')
                  )}
                >
                  <div className="mb-2 flex items-center">
                    {themeValidation.wcagAAA ? (
                      ...
                      <svg className="text-warning mr-2 h-5 w-5" fill="currentColor" viewBox="0 0 20 20">
                        <path
                          fillRule="evenodd"
                      ...
                      <svg className="text-destructive mr-2 h-5 w-5" fill="currentColor" viewBox="0 0 20 20">
                        <path
                          fillRule="evenodd"
                  ...
                  <p className={cn('text-sm', getContrastClassName('text-foreground', 'contrast-text'))}>
                    {themeValidation.wcagAAA
              ...
              >
                <div>
                  ...
                  <ul className="space-y-1">
                    <li>
                      <kbd className="bg-muted rounded px-2 py-1 font-mono text-xs">Tab</kbd> フォーカス移動
                    </li>
                    <li>
                      <kbd className="bg-muted rounded px-2 py-1 font-mono text-xs">Enter</kbd> 選択/実行
                    </li>
                    <li>
                      <kbd className="bg-muted rounded px-2 py-1 font-mono text-xs">Esc</kbd> キャンセル
                    </li>
                    <li>
                      <kbd className="bg-muted rounded px-2 py-1 font-mono text-xs">Space</kbd> 詳細情報
                    </li>
                  </ul>
                </div>
                ...
                <div>
                  ...
                  <ul className="space-y-1">
                    <li>
                      <kbd className="bg-muted rounded px-2 py-1 font-mono text-xs">↑↓←→</kbd> 日付・時間移動
                    </li>
                    <li>
                      <kbd className="bg-muted rounded px-2 py-1 font-mono text-xs">Delete</kbd> イベント削除
                    </li>
                    <li>
                      <kbd className="bg-muted rounded px-2 py-1 font-mono text-xs">F1</kbd> ヘルプ表示
                    </li>
                    <li>
                      <kbd className="bg-muted rounded px-2 py-1 font-mono text-xs">Home/End</kbd> 時間端移動
                    </li>
                  </ul>
                </div>
              </div>
