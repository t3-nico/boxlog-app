...
export function useEventUpdate({ onEventUpdate, events, date }: UseEventUpdateProps) {
  ...
  const calculateEventDuration = useCallback((eventId: string, eventDuration?: number) => {
  ...
  const handleEventUpdateToast = useCallback(async (
  ...
  }, [onEventUpdate, calculateEventDuration, handleEventUpdateToast, date])

  const executeEventResize = useCallback(async (
    eventId: string,
    newHeight: number,
    hasMoved = false
  ) => {
