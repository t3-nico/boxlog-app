...
| `useAppAsideStore` | アサイド種別・サイズ（`@/features/navigation`） | Yes |
...
| `plan` | プラン一覧（ソート・検索付き） |
| `record` | レコード一覧 |
| `stats` | 統計サマリー（`@/features/stats`） |
| `chat` | AIチャット |
| `none` | アサイド非表示 |

`useAppAsideStore`（`@/features/navigation`）でサイズ・種別を管理。
...
`useEntryRealtime` で Supabase Realtime を購読。Entries / Tags の変更をリアルタイムに反映。
