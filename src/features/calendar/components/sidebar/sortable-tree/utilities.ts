...
// SSR対応: navigatorが存在しない場合はfalse
export const iOS = typeof navigator !== 'undefined' && /iPad|iPhone|iPod/.test(navigator.platform);
...
export function getProjection(
  ...
  /** 最大深さ制限（タグは1で2階層、見本はundefinedで無制限） */
  ...
  const previousItem = newItems[overItemIndex - 1] as FlattenedItem | undefined;
  const nextItem = newItems[overItemIndex + 1] as FlattenedItem | undefined;
  const dragDepth = getDragDepth(dragOffset, indentationWidth);
  const projectedDepth = activeItem.depth + dragDepth;
  let maxDepth = getMaxDepth({
    previousItem,
  });
  // maxDepthLimit が指定されている場合は制限を適用
  if (maxDepthLimit !== undefined) {
    maxDepth = Math.min(maxDepth, maxDepthLimit);
  }
