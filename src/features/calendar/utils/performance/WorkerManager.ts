...
export class WorkerManager {
    ...
    for (let i = 0; i < workerCount; i++) {
      try {
        const worker = new Worker(
          new URL('../../workers/planProcessor.worker.ts', import.meta.url),
          {
            type: 'module',
          },
        );
  ...
  private handleWorkerMessage(e: MessageEvent): void {
    ...
    if (performance) {
      ...
      this.workerStats.memoryUsage = Math.max(
        this.workerStats.memoryUsage,
  ...
  async optimizeLayout(
    ...
    layouts: Array<{ id: string; x: number; y: number; width: number; height: number }>;
  }> {
   ...
   */
  async executeBatch(
    tasks: Array<{ type: string; payload: unknown; priority?: number }>,
  ): Promise<unknown[]> {
