        ...
        moveCard: (event: DragEvent) => {
          set((state) => {
            ...
            const sourceColumn = state.activeBoard.columns.find(
              (col) => col.id === event.sourceColumnId,
            );
            const targetColumn = state.activeBoard.columns.find(
              (col) => col.id === event.targetColumnId,
            );
            ...
            return {
              ...
              boards: state.boards.map((board) =>
                board.id === state.activeBoard?.id
                  ? { ...state.activeBoard, updatedAt: new Date() }
                  : board,
              ),
            };
          });
              ...
              state: {
                ...
                activeBoard: state.activeBoard
                  ? {
                      ...state.activeBoard,
                      createdAt: new Date(
                        (state.activeBoard as unknown as KanbanBoardForStorage).createdAt,
                      ),
                      updatedAt: new Date(
                        (state.activeBoard as unknown as KanbanBoardForStorage).updatedAt,
                      ),
                      columns: (state.activeBoard as unknown as KanbanBoardForStorage).columns.map(
                        (col) => ({
                          ...col,
                          cards: col.cards.map((card) => ({
                            ...card,
                ...
                selectedCard: state.selectedCard
                  ? {
                      ...state.selectedCard,
                      createdAt: new Date(
                        (state.selectedCard as unknown as KanbanCardForStorage).createdAt,
                      ),
                      updatedAt: new Date(
                        (state.selectedCard as unknown as KanbanCardForStorage).updatedAt,
                      ),
                      dueDate: (state.selectedCard as unknown as KanbanCardForStorage).dueDate
                        ? new Date(
                            (state.selectedCard as unknown as KanbanCardForStorage)
                              .dueDate as string,
                          )
                        : undefined,
                    }
                    ...
                    cards: col.cards.map((card) => ({
                      ...
                      updatedAt:
                        card.updatedAt instanceof Date
                          ...
                          : card.updatedAt,
                      dueDate:
                        card.dueDate instanceof Date ? card.dueDate.toISOString() : card.dueDate,
                      startTime:
                        card.startTime instanceof Date
                          ? card.startTime.toISOString()
                          : card.startTime,
                      endTime:
                        card.endTime instanceof Date ? card.endTime.toISOString() : card.endTime,
                    })),
                  })),
                })),
                activeBoard: state.activeBoard
                        ...
                        cards: col.cards.map((card) => ({
                          ...
                          startTime:
                            card.startTime instanceof Date
                              ...
                              : card.startTime,
                          endTime:
                            card.endTime instanceof Date
                              ? card.endTime.toISOString()
                              : card.endTime,
                        })),
                      })),
                    }
