 ...
 * IP単位の失敗ログイン試行数を取得
 */
async function getIpFailedAttempts(supabase: TypedSupabaseClient, ipAddress: string): Promise<number> {
  try {
 ...
 */
async function getIpLastFailedAttemptTime(supabase: TypedSupabaseClient, ipAddress: string): Promise<Date | null> {
  try {
    const { data, error } = await supabase
      .from('login_attempts')
      .select('attempt_time')
 ...
 * IPレート制限ステータスをチェック
 */
export async function checkIpRateLimit(supabase: TypedSupabaseClient, ipAddress: string): Promise<IpRateLimitStatus> {
