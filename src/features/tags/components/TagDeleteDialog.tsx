'use client'

import { useEffect, useState } from 'react'

import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
  AlertDialogContent,
  AlertDialogFooter,
  AlertDialogHeader,
  AlertDialogTitle,
} from '@/components/ui/alert-dialog'
import { Button } from '@/components/ui/button'
import { Input } from '@/components/ui/input'
import { Label } from '@/components/ui/label'
import type { TagUsage, TagWithChildren } from '@/types/tags'
import { AlertTriangle, Archive } from 'lucide-react'

interface TagDeleteDialogProps {
  tag: TagWithChildren | null
  onClose: () => void
  onConfirm: () => Promise<void>
  onArchive?: (tag: TagWithChildren) => Promise<void>
}

export function TagDeleteDialog({ tag, onClose, onConfirm, onArchive }: TagDeleteDialogProps) {
  const [confirmText, setConfirmText] = useState('')
  const [usage, setUsage] = useState<TagUsage | null>(null)
  const [isLoading, setIsLoading] = useState(false)
  const [isDeleting, setIsDeleting] = useState(false)
  const [isArchiving, setIsArchiving] = useState(false)

  // タグの使用状況を取得
  useEffect(() => {
    if (!tag) {
      setUsage(null)
      setConfirmText('')
      return
    }

    const fetchUsage = async () => {
      setIsLoading(true)
      try {
        const response = await fetch(`/api/tags/${tag.id}?usage=true`)
        if (response.ok) {
          const data = await response.json()
          setUsage(data.usage || { ticketCount: 0, eventCount: 0, taskCount: 0, totalCount: 0 })
        } else {
          // エラー時はデフォルト値
          setUsage({ ticketCount: 0, eventCount: 0, taskCount: 0, totalCount: 0 })
        }
      } catch (error) {
        console.error('Failed to fetch tag usage:', error)
        setUsage({ ticketCount: 0, eventCount: 0, taskCount: 0, totalCount: 0 })
      } finally {
        setIsLoading(false)
      }
    }

    fetchUsage()
  }, [tag])

  const handleConfirm = async () => {
    setIsDeleting(true)
    try {
      await onConfirm()
    } finally {
      setIsDeleting(false)
    }
  }

  const handleArchive = async () => {
    if (!tag || !onArchive) return
    setIsArchiving(true)
    try {
      await onArchive(tag)
      onClose()
    } finally {
      setIsArchiving(false)
    }
  }

  const requiresConfirmation = (usage?.totalCount || 0) > 50
  const canDelete = !requiresConfirmation || confirmText === tag?.name

  return (
    <AlertDialog open={!!tag} onOpenChange={(open) => !open && onClose()}>
      <AlertDialogContent className="max-w-3xl gap-0 p-6">
        <AlertDialogHeader className="mb-4">
          <AlertDialogTitle>タグ「{tag?.name}」を完全に削除しますか？</AlertDialogTitle>
        </AlertDialogHeader>

        <div className="space-y-3">
          {/* 警告 */}
          <div className="bg-destructive/10 text-destructive border-destructive/20 flex items-center gap-2 rounded-lg border p-3">
            <AlertTriangle className="h-4 w-4 shrink-0" />
            <p className="text-sm font-medium">この操作は元に戻せません</p>
          </div>

          {/* 使用状況 */}
          {isLoading ? (
            <div className="bg-muted flex items-center justify-center rounded-lg p-4">
              <div className="h-5 w-5 animate-spin rounded-full border-b-2 border-blue-600"></div>
            </div>
          ) : usage ? (
            <div className="bg-muted rounded-lg p-4">
              <p className="mb-2 text-sm font-medium">影響を受けるアイテム:</p>
              <ul className="text-muted-foreground space-y-1 text-sm">
                <li>• Tickets: {usage.ticketCount}件</li>
                <li>• Events: {usage.eventCount}件</li>
                <li>• Tasks: {usage.taskCount}件</li>
              </ul>
              <p className="text-muted-foreground mt-2 text-sm font-medium">合計: {usage.totalCount}件</p>
            </div>
          ) : null}

          {/* 削除後の処理 */}
          <div className="space-y-2">
            <p className="text-sm font-medium">削除すると:</p>
            <ul className="text-muted-foreground space-y-1 text-sm">
              <li>✓ タグ「t-{tag?.tag_number}」は永久に削除されます</li>
              <li>✓ 既存アイテムからこのタグが外れます</li>
              <li>✓ 統計データからも除外されます</li>
            </ul>
          </div>

          {/* 確認入力（使用件数が50件を超える場合） */}
          {requiresConfirmation && (
            <div className="space-y-2">
              <Label htmlFor="confirm-input" className="text-sm font-medium">
                確認のため、タグ名「{tag?.name}」を入力してください
              </Label>
              <Input
                id="confirm-input"
                placeholder={tag?.name}
                value={confirmText}
                onChange={(e) => setConfirmText(e.target.value)}
                className="font-mono"
              />
            </div>
          )}
        </div>

        <AlertDialogFooter className="mt-6 !justify-between">
          {onArchive ? (
            <Button variant="outline" onClick={handleArchive} disabled={isDeleting || isArchiving}>
              <Archive className="mr-2 h-4 w-4" />
              {isArchiving ? 'アーカイブ中...' : 'アーカイブ'}
            </Button>
          ) : (
            <div />
          )}
          <div className="flex gap-2">
            <AlertDialogCancel disabled={isDeleting || isArchiving}>キャンセル</AlertDialogCancel>
            <AlertDialogAction
              onClick={handleConfirm}
              disabled={!canDelete || isDeleting || isArchiving}
              className="bg-destructive text-destructive-foreground hover:bg-destructive/90"
            >
              {isDeleting ? '削除中...' : '完全に削除'}
            </AlertDialogAction>
          </div>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
  )
}
