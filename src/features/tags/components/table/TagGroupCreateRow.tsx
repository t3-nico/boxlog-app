...
export interface TagGroupCreateRowHandle {
  /** 作成モードを開始 */
  startCreate: () => void;
  /** 作成モードをキャンセル */
  cancelCreate: () => void;
...
interface TagGroupCreateRowProps {
  /** 列数（colSpan用） */
  columnCount: number;
  /** 作成完了時のコールバック */
  onCreated?: () => void;
 ...
 * グループインライン作成行コンポーネント
 *
 * テーブル最下部に表示される新規グループ作成フォーム
 * - refを使用して外部から作成開始/キャンセルを制御可能
 * - Enterで保存、Escapeでキャンセル
 * - 外側クリックでキャンセル
 */
export const TagGroupCreateRow = forwardRef<TagGroupCreateRowHandle, TagGroupCreateRowProps>(
  ({ columnCount, onCreated }, ref) => {
        ...
        >
          <TableCell colSpan={columnCount}>
            <div className="text-muted-foreground flex items-center gap-2 py-1">
              ...
              <span className="text-sm">{t('group.createInline')}</span>
            </div>
          </TableCell>
        </TableRow>
      );
...
TagGroupCreateRow.displayName = 'TagGroupCreateRow';
