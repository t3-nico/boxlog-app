...
interface TagListGroupHeaderProps {
  groupKey: string;
  groupLabel: string;
  count: number;
  isCollapsed: boolean;
 ...
 * - 削除ボタン: ホバーで表示
 */
export function TagListGroupHeader({
  groupKey,
  groupLabel,
  count,
  isCollapsed,
  ...
  return (
    <div
      role="row"
      className="bg-surface-container hover:bg-state-hover group flex cursor-pointer items-center gap-2 border-y px-3 py-2"
    ...
    >
      ...
      {isUncategorized ? (
        <FolderX className="text-muted-foreground size-4 shrink-0" />
      ) : (
