...
interface PlanTableEmptyStateProps {
  /** 列数（colSpan用） */
  columnCount: number;
  /** 全アイテム数（フィルター前） */
  totalItems: number;
 ...
 * - 完全に0件 → Create plan
 */
export function PlanTableEmptyState({
  columnCount,
  totalItems: _totalItems,
}: PlanTableEmptyStateProps) {
  const t = useTranslations('plan.emptyState');
  const { search, status, reset } = usePlanFilterStore();
