            ...
            <Tooltip key={color}>
              <TooltipTrigger asChild>
                <button
                  ...
                  className={cn(
                    'group relative h-9 w-9 shrink-0 rounded-md border-2 transition-all',
                    'hover:scale-110 hover:shadow-md',
                    'focus-visible:ring-ring focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none',
                    isSelected ? 'border-foreground scale-105 shadow-md' : 'border-border hover:border-foreground/50'
                  )}
                  style={{ backgroundColor: color }}
                  aria-label={`${colorName}を選択`}
                >
                  {isSelected && (
                    <div className="bg-background/90 absolute inset-0 flex items-center justify-center rounded-[4px]">
