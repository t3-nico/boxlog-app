...
const CommandDialog = ({
  ...
  return (
    <Dialog {...props}>
      ...
      <DialogContent className={cn(className, 'overflow-hidden p-0')} showCloseButton={showCloseButton}>
...
const CommandInput = ({ className, ...props }: React.ComponentProps<typeof CommandPrimitive.Input>) => {
  return (
    <div data-slot="command-input-wrapper" className="border-b-border flex h-10 items-center gap-2 border-b px-3">
...
const CommandItem = ({ className, ...props }: React.ComponentProps<typeof CommandPrimitive.Item>) => {
  return (
    <CommandPrimitive.Item
      ...
      className={cn(
        "data-[selected=true]:bg-state-selected [&_svg:not([class*='text-'])]:text-muted-foreground relative flex cursor-default items-center gap-2 rounded-sm px-2 py-2 text-sm outline-hidden select-none data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}
    />
