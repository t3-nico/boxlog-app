...
function MobileSidebar({ open, close, children }: React.PropsWithChildren<{ open: boolean; close: () => void }>) {
  return (
    <Headless.Dialog open={open} onClose={close} className="lg:hidden">
      ...
      >
        <div className="flex h-full flex-col rounded-lg bg-gray-50 shadow-xs dark:bg-gray-900">
...
export function SidebarLayoutCustom({
  navbar,
  sidebar,
  children,
  collapsed = false,
  ...
  return (
    <div className="relative isolate flex h-full w-full max-lg:flex-col">
      {/* Sidebar on desktop */}
      <div
        className={clsx(
          'max-lg:hidden transition-all duration-150 h-full flex-shrink-0',
          collapsed ? 'w-16' : 'w-64'
        )}
      >
      ...
      <main className="flex flex-1 flex-col lg:min-w-0">
        <div className="grow lg:bg-white dark:lg:bg-gray-800 h-full">
          {children}
        </div>
      </main>
    </div>
