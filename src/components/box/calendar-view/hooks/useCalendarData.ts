...
import type { ViewDateRange } from '../types'
...
export function useCalendarData({ 
  ...
  // タスクの期間変更
  const resizeTask = useCallback(async (taskId: string, newDuration: number) => {
    if (!onTaskUpdate) return
  ...
  // 新規タスク作成
  const addTask = useCallback(async (date: Date, time: string) => {
  ...
  sorted.forEach(task => {
    ...
    for (let col = 0; col < columns.length; col++) {
      const canPlace = columns[col].every(existing => {
        // 重複チェック
        return task.displayStart >= existing.displayEnd ||
               task.displayEnd <= existing.displayStart
      })
...
export function detectTimeConflicts(
  ...
  return tasks.filter(task => {
    ...
    return !(
      targetTask.displayEnd <= task.displayStart ||
      targetTask.displayStart >= task.displayEnd
    )
  })
}
