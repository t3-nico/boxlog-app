...
test.describe('BoxLog App - Plan Creation Flow', () => {
  ...
  test('プラン一覧ページが表示される', async ({ page }) => {
    ...
    const isValidPage = finalUrl.includes('board') || finalUrl.includes('login') || finalUrl.includes('auth')
  ...
  test('プランカードのインタラクションが機能する', async ({ page }) => {
    ...
    // プランカードを探す
    const planCard = page.locator('[data-testid="plan-card"], [class*="plan-card"], [class*="task-card"]').first()
...
test.describe('BoxLog App - Plan Status Management', () => {
  ...
  test('ステータスカラム（todo/doing/done）が表示される', async ({ page }) => {
    ...
    const hasStatusColumns =
      (await todoColumn.count()) > 0 || (await doingColumn.count()) > 0 || (await doneColumn.count()) > 0
