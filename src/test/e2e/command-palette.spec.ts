...
test.describe('BoxLog App - Command Palette', () => {
  ...
  test('コマンドパレットでキーボードナビゲーションが機能する', async ({ page }) => {
    ...
    try {
      ...
      // 選択状態が変わることを確認
      const selectedItem = page.locator('[aria-selected="true"], [data-selected="true"], [class*="selected"]')
...
test.describe('BoxLog App - Command Palette Accessibility', () => {
  ...
  test('コマンドパレットにARIA属性が設定されている', async ({ page }) => {
    ...
    try {
      ...
      console.log(`ARIA roles - dialog: ${dialogCount}, combobox: ${comboboxCount}, listbox: ${listboxCount}`)
