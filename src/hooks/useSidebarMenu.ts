...
import { MenuItem, MenuSection, sidebarConfig } from '@/config/ui/sidebarConfig'
    ...
    return sidebarConfig
      ...
      .map((section) => {
        ...
        let items = section.items.filter(
          (item) => canAccessItem(item, user) && !preferences.hiddenSections.includes(item.id)
        )
...
// アクセス権限チェック関数
function canAccessSection(section: MenuSection, user: User | null): boolean {
...
function canAccessItem(item: MenuItem, user: User | null): boolean {
  // プラン制限（現在無効化）
